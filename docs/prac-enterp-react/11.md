...
**@fullcalendar/react** :是个连接器。它告诉核心 FullCalendar 包开始使用 react 虚拟 DOM Node而不是它通常使用的 Preact Node进行渲染，将 FullCalendar 转换为“真正的”React 组件。
...
接下来，打开 Redux DevTools。确保选择顶部下拉箭头上的日历–React样本。
...
在下一章，我们将构建登录和注册表单。我们将需要假Node json-server 和 json-server-auth 的帮助，以及来自优秀 Material-UI 的更多样式组件。