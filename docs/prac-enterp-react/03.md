...
在前一章中，我们学习了如何使用Node包管理器或 NPM，它是一个命令行工具，允许我们安装和更新应用包。

本章将研究 React 函数组件，这已经成为编写最新 React 应用的现状，以及使用 TypeScript 定义函数组件所用道具的语法。简单地说，React 函数组件本质上是返回 JSX 或 JavaScript XML 的 JavaScript 函数。
...
NPX(Node包执行)是 NPM 自带的。NPX 对于一次性软件包很有用，它从 NPM 注册表执行软件包，而不在本地安装它们。
...
进入你的浏览器，点击标题“实际React企业”来更新本地状态标题。
