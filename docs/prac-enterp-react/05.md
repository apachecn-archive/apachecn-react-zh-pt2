...
每当我们需要通过我们的 React 应用在多个视图中导航时，路由器都会管理 URL。React Router 使我们的应用 UI 和 URL 彼此保持同步。这些好处包括:
...
让我们开始为我们的应用构建 React 路由器导航。首先，让我们对一些默认文件进行一些编辑，然后一步一步地重新构建它。
...
如果使用 VS 代码，打开文件`Home.tsx`并输入代码片段`rafce`(React箭头函数导出组件的缩写)。在 WebStorm 中，片段是`rsc`(React 无状态组件的简称，没有 prop 类型和 ES6 模块系统)。
