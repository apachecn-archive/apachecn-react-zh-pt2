...
`Selector`组件是一个容器，它为每个子组件呈现一个按钮，并且只显示用户选择的那个按钮。为了提供将由应用程序显示的数据以及对其进行操作的回调函数的实现，我向`src`文件夹添加了一个名为`ProductsAndSuppliers.js`的文件，并使用它来定义清单 [18-14](#PC14) 中所示的组件。
