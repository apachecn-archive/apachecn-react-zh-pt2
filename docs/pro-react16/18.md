...
| React还原 | React Redux 通过其 props 将 React 组件连接到 Redux 数据存储，允许直接访问数据，而不依赖于 prop 线程。我在第 19 章和第 20 章中使用了这个包。 |
| React路由器 | React 路由器为 React 应用提供 URL 路由，允许根据浏览器的 URL 选择显示给用户的组件。我在第 [21](21.html) 章和第 [22](22.html) 章使用这个包。 |
...
| 阿波罗React | React Apollo 将 React 组件连接到 GraphQL 查询和变异，允许通过 props 消费 GraphQL 服务。 |
