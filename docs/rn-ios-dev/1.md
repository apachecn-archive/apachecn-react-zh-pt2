...
这里你用一个名为`myProp`的道具初始化 App 组件，这个道具的目标是`'container'`的 id。您可以通过以下方式在 JSX 通过插值访问此道具。让我们创建一个组件应用:
