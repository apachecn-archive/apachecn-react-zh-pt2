...
在这里的`src/main.js`中，我们使用变量`React`来获取`react,`，我们使用变量`App`来获取来自`./components/app.js`的组件。我们在目标 id `main`上渲染`App`组件——所有Flux的魔法都在这里发生。接下来，我们通过在`src/components/app.js`中添加以下代码来创建我们的组件:
...
同样，我们很少有`require`语句来抓取React、动作和存储。接下来，使用`createClass`我们创建一个`AppTitle`组件，它有一个带有`div`的渲染方法，该方法有一个带有`onClick`事件的`h3`标签。我们还有另一个函数`onClickHandler`，每当点击`h3`标签时，就会触发一个`onClick`事件。
