...
如果您还记得，在之前的练习中，我们使用路由器构建了我们的应用。我们的路由器页面正在扩展`RouteComponentProps`。这很有用，因为我们能够使用路由器 API 并提取路由器页面的名称。例如，对于文章页面，我们设置了这个接口签名，它是通过`generate-react-cli`为我们创建的模板页面。
