...
`ThreadStore`和`MessageStore`一样，只有公共的 getter 方法，没有 setter 方法。`ThreadStore`向调度程序注册一个回调，其中包含`switch`语句，该语句将控制存储如何对调度程序发送的动作做出React。`switch`语句响应通过调度程序发送的特定`ActionTypes`，然后发送`emitChange()`事件。
