...
如果您运行应用并访问浏览器，您应该会看到“React 的 Hello”印在窗户上。到目前为止，我们还没有在浏览器上显示时间。让我们使用 React 道具来实现这一点。
...
*   可以使用函数组件创建一个简单的客户端 React 应用。我们可以将道具传递给组件，然后组件可以将道具数据呈现给浏览器窗口。
