...
当你的组件被初始化时,`created`钩子运行。您将能够访问React数据，并且事件是活动的。你可以使用清单 [9-5](#PC5) 中的方法。
...
`mounted`钩子提供了对React组件、模板和呈现的 DOM ( `via. this.$el`)的完全访问。你可以使用清单 [9-7](#PC7) 中的方法。
...
Vue Router 是一个用于单页面应用的官方路由插件，设计用于 Vue.js 框架内。路由器是在单页应用中从一个视图跳到另一个视图的一种方式，无需刷新 web 浏览器。在 VueNoteApp 中集成 Vue 路由器很容易。
...
*   Vue Router 是一个用于单页面应用的官方路由插件，设计用于在`Vue.js`框架内使用。路由器是在单页应用中从一个视图跳到另一个视图的一种方式。
